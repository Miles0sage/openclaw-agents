...
# Check if any tool is in multiple profiles
tool_counts = {}
for agent, profile in AGENT_TOOL_PROFILES.items():
    for tool in profile:
        if tool not in tool_counts:
            tool_counts[tool] = []
        tool_counts[tool].append(agent)

print("\nTools in multiple profiles (sample):")
for tool, agents in list(tool_counts.items())[:10]:
    print(f"  - {tool}: {agents}")
